%This script is a WM EXPERIMENT RAN AT THE SCANNER FROM MY AIR!!!!!!!!!!!!!
...
%
%Currently a run is 280.8 seconds (4 mins and 40.8 secs)
%TR is 800 ms, oversampling at 1 sample per 50 ms
%Must use 351 TR's for this run
...
%try
    ...
    Screen('Preference', 'SkipSyncTests', 1); % HK added to avoid screen sync error freeze
    ...
    ScreenHeight = 90; % in cm, 90 cm in the scanner!!!
    ViewDistance = 370; % in cm, 370 cm in the scanner!!! (57 cm is the ideal distance where 1 cm equals 1 visual degree)
    ...
    %----------------------------------------------------------------------
    cd(datadir); 
    if exist(['WM_DistractV2_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
        load(['WM_DistractV2_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
        ...
        %Currently, MinTrialNum is 108. Running 12 trial blocks thus means
    ...
    t.BeginFixation = 12.8 + 3; %16 TRs need to be extra (16trs * .8ms)
    t.EndFixation = 3;
    ...
    p.PatchSize = round(.25*7*p.ppd); %Size of the patch that is drawn on screen location, so twice the radius, in pixels
    ...
    for n = 1:p.NumTrials
        ...
        while RespTimePassed<t.ResponseTime  %As long as no correct answer is identified
            ...
            if keyCode(KbName('b')) %BIG step CCW
                test_orient = rem(test_orient-2+1440,180);
            elseif keyCode(KbName('y')) %small step CCW
                test_orient = rem(test_orient-.5+1440,180);
            elseif keyCode(KbName('g')) %small step CW
                test_orient = rem(test_orient+.5+1440,180);
            elseif keyCode(KbName('r')) %BIG step CW
                test_orient = rem(test_orient+2+1440,180);
        ...
        %% iti
        if n~=p.NumTrials %if not the last trial, change the fixation to predict the distractor type forthcoming
            fix_predict_col = eval(['p.CueColors.' char(TrialStuff(p.TrialNumGlobal+1).distractorname)]);
    ...
    eval(['save(''WM_DistractV2_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-v7.3'')']);
    cd(expdir)
