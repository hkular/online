...
%% 
function WM_DistractV11practiceFast(p, info, nruns, startRun)
        ...
        %% Target rendering
          Screen('BlendFunction', window, 'GL_SRC_ALPHA', 'GL_ONE_MINUS_SRC_ALPHA'); % add transparency with alpha blending
            for i = 1:4
                ...
                Screen('DrawTexture', window, MaskToDraw, [], MyPatch{i}, [], 0, 0.2); % alpha = 0.5 in experiment, 0.2 in practice
            end
...
while (feedbackTimePassed<t.FeedbackTime)
      if data.Response(n) == TrialStuff(p.TrialNumGlobal).correctresp
            % correct display
            Screen('FillRect',window,p.MyGrey);
            Screen('FillOval', window, green, [CenterX-p.FixRadius CenterY-p.FixRadius CenterX+p.FixRadius CenterY+p.FixRadius])
            Screen('DrawingFinished', window);
            Screen('Flip', window);
      elseif data.Response(n) ~= TrialStuff(p.TrialNumGlobal).correctresp
            % incorrect display
            Screen('FillRect',window,p.MyGrey);
            Screen('FillOval', window, red, [CenterX-p.FixRadius CenterY-p.FixRadius CenterX+p.FixRadius CenterY+p.FixRadius])
            Screen('DrawingFinished', window);
            Screen('Flip', window);
      end
