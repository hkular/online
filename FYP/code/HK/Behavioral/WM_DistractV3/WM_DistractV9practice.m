...
%% 
function WM_DistractV9practice(p, info, nruns, startRun)
...
cd(datadir); 
    if exist(['WM_DistractV9Practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        load(['WM_DistractV9Practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
         ...
         while RespTimePassed<t.ResponseTime  %As long as no correct answer is identified
          ...
          if sum(keyCode)==1
            ...
            elseif keyCode(KbName('ESCAPE')) % If user presses ESCAPE, exit the program.
                ...
                eval(['save(''WM_DistractV9Practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-v7.3'')']);
                cd(expdir)
