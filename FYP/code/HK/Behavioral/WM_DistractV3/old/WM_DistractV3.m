...
%% 
function WM_DistractV3(nruns, startRun)

Screen('Preference', 'SkipSyncTests', 1);

% Inputs
% nruns: number of runs to execute sequentially (default is 3)
% startRun: run number to start with if interrupted (default is 1)
% block: block number

% Stimulus presentation
% 4 rings, 1 in each quadrant, center fixation, center of each ring is 7
% deg periphery, 
...
% what system are we on and adjust for it's needs
if IsOSX == 1
    AssertOpenGL;
elseif IsLinux == 1
    OpenGL;
else IsWin == 1
    warning('find out what Win needs');
end
 ...
 %% Prepare and collect basic info
    ...
    %Sub.Num = deblank((Sub.Num)); % remove null characters
...
cd(datadir); 
    if exist(['WM_DistractV3_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        load(['WM_DistractV3_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
        ...
        p.StartRun = TheData(end).p.TrialNumGlobal+1 ;
    else
        runnumber = currentBlock; %If no data file exists this must be the first run
        p.Block = currentBlock;
        p.TrialNumGlobal = 0;
        p.StartRun = 1; 
        %Experimental params required for counterbalancing
        p.NumOrientBins = 3; %must be multiple of the size of your orientation space (here: 180) was 6 for RR
        p.OrientBins = reshape(2:2:180,90/p.NumOrientBins,p.NumOrientBins); % 2 deg steps, 3 cols
        p.Quads = [1 2 3 4];
        p.Contrast = [.3 .8];
        %p.QuadNames = {'upper right'; 'upper left' ; 'lower left' ; 'lower right'};
        p.SetSize = [1 2 4];
        [TrialStuff, MinNumTrials] = CounterBalancingActV3(p.OrientBins, p.Quads, p.SetSize, p.Contrast);
        p.NumTrials = 21; %NOT TRIVIAL!!! --> must be divisible by MinTrialNum AND by the number of possible iti's (which is 3)
        % curerntly mintrial num is 189, running 9 blocks of trial size 21
        %Currently, MinTrialNum is 243. Running 27 trial blocks thus means
        %that 9 blocks need to be ran for one full counterbalancing act
...
%Timing params -- 
    ...
    t.BeginFixation = 0; %16 TRs need to be extra (16trs * .8ms)
    t.EndFixation = 3;
    ...
    % need smaller patch size for quadrant system, let's try 1/4
    ...
    p.FixColor = black;
    p.ResponseLineWidth = 2; %in pixel is actually 2
    ...
    p.ContrastDistGrating = .5; % is actually .5 
...
% start with a meshgrid
    ...
    % p.NumTrials = 21; % 189/9 = 21
    startTrialThisBlock = (p.NumTrials * currentBlock) - p.NumTrials + 1;
    
    for n = startTrialThisBlock:(startTrialThisBlock+p.NumTrials - 1) %this is the location of the problem but why
 ...
 AssertOpenGL;
    ...
    if gammacorrect
        ...
        CalibrationFile = 'calib_07-Nov-2016.mat'; %find behav roomb/c?
...
%% Welcome and wait for trigger
  ...
  %% delay 1
        ...
        while RespTimePassed<t.ResponseTime  %As long as no correct answer is identified
            ...
            if keyCode(KbName('UpArrow')) % b BIG step CCW
                test_orient = rem(test_orient-2+1440,180);
            elseif keyCode(KbName('LeftArrow')) % y small step CCW
                test_orient = rem(test_orient-.5+1440,180);
            elseif keyCode(KbName('RightArrow')) % g small step CW
                test_orient = rem(test_orient+.5+1440,180);
            elseif keyCode(KbName('DownArrow')) % r BIG step CW
                test_orient = rem(test_orient+2+1440,180);
    ...
    blockStr = ['Finished block ' num2str(currentBlock) ' out of 9'];
    Screen('FillOval', window, p.FixColor, [CenterX-p.OuterFixRadius CenterY-p.OuterFixRadius CenterX+p.OuterFixRadius CenterY+p.OuterFixRadius])
    Screen('Flip',window);
    %DrawFormattedText(window,[blockStr '\n\nPress the spacebar to continue'],CenterX-200,CenterY,white);
    DrawFormattedText(window,[blockStr],CenterX-200,CenterY,white);
    Screen('Flip',window);
...
%     
    ...
    targets_were = [TrialStuff(p.StartRun:p.TrialNumGlobal).testorient];
    % need to filter NaNs?
    ...
    eval(['save(''WM_DistractV3_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-v7.3'')']);
    cd(expdir)
