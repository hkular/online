...
%% 
function WM_DistractV5practice(nruns, startRun)
...
cd(datadir); 
    if exist(['WM_DistractPractice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        load(['WM_DistractPractice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
        ...
        p.Quads = [1 2 3 4];
        p.SetSize = [1 2 4];
        [TrialStuff, designMatFull, MinNumTrials] = CounterBalancingActV5practice(p.OrientBins, p.SetSize, p.Quads, p.Contrast);
...
%% Welcome and wait for trigger
  ...
  %% delay 1
    ...
    eval(['save(''WM_Distractpractice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-v7.3'')']);
    cd(expdir)
