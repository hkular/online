...
%% 
function WM_DistractV6practice(p, info, nruns, startRun)
 ...
 %% Prepare and collect basic info
    ...
    info.Speed = input('Speed of presentation:', 's'); if isempty(info.Speed); info.Gender = 'tmp'; end
    t.PhaseReverseFreq = 4; %in Hz, how often gratings reverse their phase
    t.PhaseReverseTime = .4/t.PhaseReverseFreq;
    if info.Speed == '1' % slow
        t.TargetTime = 16*t.PhaseReverseTime; 
    elseif info.Speed == '2' % fast
        t.TargetTime = 2*t.PhaseReverseTime;
    else
        error('Select a speed for practice');
    end
...
cd(datadir); 
    if exist(['WM_DistractV6Practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        load(['WM_DistractV6Practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat']);
        ...
        %p.OrientBins = reshape([(75:2:105), (165:2:195)],[], p.NumOrientBins);% 16 deg around meridians
        ...
        p.Quads = [1 2 3 4];
        p.SetSize = [1 2];
        [TrialStuff, designMatFull] = Practice_CounterBalancingActV6(p.OrientBins, p.SetSize, p.Quads);
        ...
        % total trials is 96, can do 4 practice runs, 2 at slow speed and 1
        % at fast speed, 1 extra any speed
...
%   t.PhaseReverseTime = .1/t.PhaseReverseFreq;
    %t.TargetTime = 6*t.PhaseReverseTime; %must be multiple of t.PhaseReverseTime
    %t.DelayTime = .25; %total delay in sec. shortened from 13
...
% start with a meshgrid
    ...
    % 1 or ] gratings as needed for each trial.  
    ...
    clear  n sine sine2 stim_phase1 stim_phase2 
         ...
         while RespTimePassed<t.ResponseTime  %As long as no correct answer is identified
          ...
          if sum(keyCode)==1
            ...
            elseif keyCode(KbName('ESCAPE')) % If user presses ESCAPE, exit the program.
                ...
                eval(['save(''WM_DistractV6Practice_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'', ''TheData'', ''TrialStuff'', ''-v7.3'')']);
                cd(expdir)
