...
%
%  make 2 mini design matrices, and then put them all together again at the end for your full
% experimental design.... 
%
...
designMatFull = [designMat1;designMat2]; % length 48 trials
...
% 1 is on target, 2 is off target, for practice I want this to be 1/4 of
% items
quarter_trials = length(designMatFull)/4;
test_item  = [ones(quarter_trials*3,1);ones(quarter_trials,1)*2];
% randomized
designMatTest = Shuffle(test_item);

designMatFull = [designMatFull,designMatTest]; %<- this adds 1 new column which tells you the contrast level for the distractor noise
% col1 = set size, col2 = ori1, col3 = ori2, col4 = ori3, col5 = ori4, 
% col6 = spatial config, col7 = test
%----------------------------------------------------------------------
...
for i = 1:length(designMatFull)
    ...
    if  trial.setsize == 1 % set size if full counterbalanced
        ...
        if designMatFull(trial_cnt_shuffled(i),7) == 1
            trial.test = trial.position;
        elseif designMatFull(trial_cnt_shuffled(i),7) == 2
