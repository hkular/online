...
%% 
function WM_DistractV4(nruns, startRun)
...
cd(datadir); 
    if exist(['WM_DistractV3_S', num2str(info.SubNum), '_', num2str(info.TheDate), '_Main.mat'])
        ...
        p.Quads = [1 2 3 4];
        p.SetSize = [1 2 4];
        [TrialStuff, designMatFull, MinNumTrials] = CounterBalancingActV5(p.OrientBins, p.SetSize, p.Quads, p.Contrast);
...
%Timing params -- 
    ...
    t.ResponseTime = 2; % shortened response time
...
%% Welcome and wait for trigger
  ...
  %% delay 1
    ...
    Screen('FillOval', window, p.FixColor, [CenterX-p.OuterFixRadius CenterY-p.OuterFixRadius CenterX+p.OuterFixRadius CenterY+p.OuterFixRadius])
    Screen('Flip', window);
    blockStr = ['Finished block ' num2str(currentBlock) ' out of 8'];
