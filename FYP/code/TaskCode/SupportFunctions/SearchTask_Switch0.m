...
function p = SearchTask_Switch0(p,win,curTask)
%--------------------------------------------------------------------------
...
Screen(win.onScreen,'TextSize',20); 
%----EEG stuff----------------------------------------------------------
if p.portCodes == 1
    ppdev_mex('Open', 1);
end
...
%----Prefs-----------------------------------------------------------------
prefs = getPrefs(win,p);
prefs.targetColor = p.counterbalance_col;
...
% Set up save file
%--------------------------------------------------------------------------
fileName = [p.root,filesep,'Data',filesep,num2str(p.subNum), '_search_switch0.mat'];
if p.subNum ~= 0
    if exist(fileName)
        Screen('CloseAll');
...
for b = 1:prefs.nBlocks
    ...
    data.iti(:,b) = prefs.iti(prefs.design(data.trialOrder(:,b),4));
    %----------------------------------------------------------------------
    % Trial loop
    %----------------------------------------------------------------------
    for t = 1:prefs.nTrials
        ...
        if p.counterbalance_col == 1 % target color is green
            tCol = prefs.green;
            dCol = prefs.red;
        else
            tCol = prefs.red;
        ...
        data.block_color(1,b) = p.counterbalance_col;
    ...
    if block_acc > 90 && block_rt < 750
        if curTask == 1 % eye change first
            p.blockCash(b) = .25;
        elseif curTask == 2
            p.blockCash(b + prefs.nBlocks) = .25;
        elseif curTask == 3
            p.blockCash(b + prefs.nBlocks + prefs.nBlocks) = .25;
        end
    end
    total_cash = sum(p.blockCash);
    
    text1 = sprintf('Average Accuracy = %.1f Percent',block_acc);
    text2 = sprintf('Average Speed = %.2f seconds',block_rt);
    text3 = sprintf('Accrued Bonus Cash = $%.2f',total_cash);
    ...
    if b < prefs.nBlocks
        ...
        while toc < prefs.breakLength*60
            ...
            Screen(win.onScreen, 'DrawText', ['Block ',num2str(b + ((curTask-1) * prefs.nBlocks)),' of ',num2str(prefs.nBlocks*3),' completed.'], win.centerX-110, win.centerY+20, [255 255 255]);
            ...
            Screen(win.onScreen, 'DrawText', text3, win.centerX-110, win.centerY+155, [255 255 255]);
            Screen('Flip', win.onScreen);
        end
...
%         Screen('TextFont',win.onScreen,'Arial');
        if curTask == 1
            Screen(win.onScreen, 'DrawText', '1/3 of the way there!', win.centerX-250, win.centerY-75, [255 255 255]);
        elseif curTask == 2
            Screen(win.onScreen, 'DrawText', '2/3 of the way there!', win.centerX-250, win.centerY-75, [255 255 255]);
        else
...
prefs.viewDist = 46;   % viewing distance (cm)
prefs.aspectRatio = win.screenY / win.screenX;
prefs.pxSize_x = prefs.monWidth / win.screenX; % each pixel is this many cm wide
prefs.pxSize_y = prefs.monHeight / win.screenY; % each pixel is this many cm wide
prefs.pxSize = mean([prefs.pxSize_x,prefs.pxSize_y]); % just take the mean for calculating item size (x and y pixels are slightly different sizes).
prefs.pxSize_DVA = rad2deg(atan((1/2)/prefs.viewDist*prefs.pxSize)*2);
prefs.screenX_DVA =  rad2deg(atan((win.screenX/2)/prefs.viewDist*prefs.pxSize_x)*2); % save for reference later
prefs.screenY_DVA =  rad2deg(atan((win.screenY/2)/prefs.viewDist*prefs.pxSize_y)*2); % save for reference later
prefs.ppd = 1 / prefs.pxSize_DVA; % pixels per degree

% Main size parameters
prefs.fixationSize = round(prefs.ppd * .25);
prefs.arrayRadius = round(prefs.ppd * 7); % (pixels) radius of imaginary circle that stimuli are placed on...
prefs.stimRadius = round(prefs.ppd * 2.4); % (pixels) radius of stimulus bounding box
prefs.stimRadius_circle = (pi*((prefs.stimRadius/2)^2))^(1/2); % make the circle the same AREA as the square...
prefs.lineLength = round(prefs.ppd * .8); % half of the length of the target line (vertical or horizontal)
prefs.penWidth = round(prefs.ppd * .1);
prefs.red = [255,0,0];
prefs.green = [0,255,0];
prefs.white = [255,255,255];
prefs.possibleShapes = [0 0 0 0 0 0 0]; % possible non-target and distractor shapes (0 = circle, otherwise # = # of sides in polygon);
...
prefs.distractorPresent = [0,1]; % 0 = absent, 1 = present
prefs.targetOrient = [1,2]; % 1 = horizontal, 2 = vertical
prefs.iti = [1,1.2,1.4];
prefs.nPerCond = 6; % number of repeats of design matrix to get total # of trials desired per block
