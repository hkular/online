...
function p = SearchTask_Switch100(p,win,curTask)
%--------------------------------------------------------------------------
...
Screen(win.onScreen,'TextSize',30); 
...
% Set up save file
%--------------------------------------------------------------------------
fileName = [p.root,filesep,'Data',filesep,num2str(p.subNum), '_search_switch100.mat'];
if p.subNum ~= 0
    if exist(fileName)
        Screen('CloseAll');
...
for b = 1:prefs.nBlocks
    ...
    for t = 1:prefs.nTrials
        ...
        if mod(t,2) == 0
            data.targetColor(t,b) = 1;
        else
            data.targetColor(t,b) = 2;
        end
