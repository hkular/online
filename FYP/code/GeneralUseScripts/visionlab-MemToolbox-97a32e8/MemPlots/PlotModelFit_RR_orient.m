% PLOTMODELFIT_RR_orient plots the probability density function of the model 
...
function figHand = PlotModelFit_RR_orient(model, params, data, varargin)
  % Extra arguments and parsing
  args = struct('PdfColor', [0.99, 0.2, 0.2], 'NumberOfBins', 180, ...
...
function PlotContinuousReport_RR(model, params, data, args)
  ...
  set(gcf, 'Color', [1 1 1]);
  x = linspace(-90, 90, args.NumberOfBins+1)';
  err_per_sub = reshape(data.errors,100,12);
  for n = 1:12
...
%   bar(x, normed_n, 'EdgeColor', [1 1 1], 'FaceColor', [.8 .8 .8]);
  ...
  hold on;
  set(gca, 'XTick',[-90 -45 0 45 90],'YTick',[0 .01 .02 .03 .04 .05 .06 .07],'LineWidth',2,'Box','off','YGrid','on','Ylim',[0 .072],'Xlim',[-90 90]);

  % Plot scaled version of the prediction
  vals = linspace(-90, 90, 100)';
