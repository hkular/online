...
fixColor = [255 255 255]; % fixation marker color
...
viewTimeImage = Trials.onset(2)-0.05;
% fix RT issue try
% viewTimeImage = Trials.onset(2)+0.1
...
picPtrs = [];
catDirs = {'word' 'word-colored' 'limb' 'limb-colored' 'adult' 'adult-colored' 'house' 'house-colored' 'instrument' 'instrument-colored'};
for t = 1:numTrials
    cd(path.stimDir);
...
for t = 1:numTrials
    ...
    %[keys RT] = recordKeys(trialStart,viewTimeImage,k);
    %[keys RT] = recordKeys(startTime+(t-1)*viewTime,viewTimeImage,k);
    [keys RT] = recordKeys(startTime+(t-1)*viewTime,waitDur,k);
    data.keys{t} = keys;
    data.rt(t) = min(RT);
    ...
    WaitSecs(viewTime-viewTimeImage);
